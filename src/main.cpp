...
DMD_RGB_FM6363<RGB128x64plainS32, COLOR_4BITS> dmd(mux_list, DMD_PIN_nOE, DMD_PIN_SCLK, custom_rgbpins, DISPLAYS_ACROSS, DISPLAYS_DOWN, ENABLE_DUAL_BUFFER);
...
void loop(void)
{
    ...
    while (1)
    {
        if ((millis() - prev_step) > interval)
        {
            ...
            case 0:
                if (dmd.stepMarquee(step[i], 0) & 1)
                { // if text is reached screen bounds
            ...
            case 2:
                ...
                { // if text is reached screen bounds
                    ...
                    {
                        if (step[i] < 0)
                            dmd.drawMarqueeX(m, 0, dmd.height());
                        else
                            dmd.drawMarqueeX(m, 0, 0);
                    }
                }
